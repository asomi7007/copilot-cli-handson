...
4. **나만의 규칙(지침) 만들기** — Copilot이 항상 따를 규칙을 자연어로 만든다
5. **나만의 기능(스킬) 만들기** — "내 게임을 갤러리에 올리는 기능"을 직접 만든다

모두 **터미널에 자연어로 말하면** 됩니다.

> 게임은 재미있는 연습 소재일 뿐입니다. **똑같은 방법으로 문서 작성, 파일 정리, 자료 조사, 반복 업무를 자동화**할 수 있습니다. 이 실습의 진짜 목적은 "코딩을 몰라도 자연어로 컴퓨터에 일을 시킨다"를 손으로 체감하는 것입니다.
